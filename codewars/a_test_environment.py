...
def who_is_winner(pieces_position_list: list) -> str:
	print(pieces_position_list, len(pieces_position_list), sep='\n', end='\n')
	...
	for move, color in list(enumerate(pieces_position_list, start=1)):
		...
		pd_board.at[col_dict[color[0]][0], color[0]] = move, color  # a tuple to match the board
		...
		if move >= 7:
			...
			if len(the_col_series) >= 4:
				...
				col_count = the_counting_list(col_list_for_win, color[2:])
				if col_count is not None:
					return col_count
			# check rows for a win
			the_row_series = pd_board[pd_board.apply(lambda row: row[color[0]] == (move, color), axis=1)].iloc[
				0].tolist()
			if len(the_row_series) >= 4:
				row_list_for_win = [i[1][2:] for i in the_row_series]
				row_count = the_counting_list(row_list_for_win, color[2:])
