def is_interesting(number, awesome_phrases):
	...
	try:
		if number == int(''.join([str(i) for i in range(int(str(number)[0]), len(str(number)) + 1)])):
			return 2
	except ValueError:  # handle the case where the number is 7890 0 should come after 9, and not before  1, as in 7890.
		pass
	# The digits are sequential, decrementing‡: 4321
	try:
		if number == int(''.join([str(i) for i in range(a, 0, -1)])):
			return 2
	except ValueError:  # handle the case where the number is 3210 0 should come after 1, and not before  9, as in 3210.
		pass
